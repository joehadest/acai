...
const CategoriesTab = ({ categories: initialCategories, onUpdate }: { categories: { _id?: string; value: string; label: string; order?: number }[]; onUpdate: () => void }) => {
  ...
  return (
    <div className="bg-[#262525] rounded-xl p-6 border border-gray-800">
      ...
      {catError && <p className="text-purple-500 text-sm mb-4">{catError}</p>}
          ...
          <tbody>
            {categories.sort((a, b) => (a.order || 0) - (b.order || 0)).map(cat => (
              <tr key={cat._id} className="border-b border-gray-700">
                ...
                <td className="p-2">
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${cat.allowHalfAndHalf
...
export default function AdminMenu() {
  ...
  return (
    <div className="bg-[#1a1a1a] min-h-screen text-gray-200 p-4 sm:p-6 lg:p-8">
      <style jsx global>{`
  .form-input { @apply w-full mt-1 p-2 bg-[#2a2a2a] border border-gray-600 rounded-md text-white focus:ring-purple-500 focus:border-purple-500 transition-colors; }
        .form-label { @apply block text-sm font-medium text-gray-300; }
  .form-checkbox { @apply h-4 w-4 rounded border-gray-600 bg-gray-700 text-purple-600 focus:ring-purple-500; }
  .form-button-primary { @apply px-4 py-2 bg-purple-600 rounded-md hover:bg-purple-700 transition-colors flex items-center justify-center font-semibold; }
        .form-button-secondary { @apply px-4 py-2 bg-gray-600 rounded-md hover:bg-gray-700 transition-colors; }
  .form-button-danger { @apply p-2 bg-purple-900 rounded-md hover:bg-purple-700 transition-colors; }
      `}</style>
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
          ...
          <div className="flex gap-2 mt-4 sm:mt-0">
            <button className={`px-4 py-2 rounded-lg font-semibold flex items-center gap-2 transition-colors ${activeTab === 'menu' ? 'bg-purple-600 text-white' : 'bg-[#2a2a2a] text-gray-300 hover:bg-purple-700'}`} onClick={() => setActiveTab('menu')}> <FaThList /> Itens</button>
            <button className={`px-4 py-2 rounded-lg font-semibold flex items-center gap-2 transition-colors ${activeTab === 'categories' ? 'bg-purple-600 text-white' : 'bg-[#2a2a2a] text-gray-300 hover:bg-purple-700'}`} onClick={() => setActiveTab('categories')}> <FaListAlt /> Categorias</button>
          </div>
        </div>
        ...
        {activeTab === 'menu' && (
          <div>
            ...
            {loading ? <p className="text-center py-10">Carregando...</p> : error ? <p className="text-purple-500 text-center py-10">{error}</p> : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredItems.map((item) => (
                  <motion.div key={item._id} layout className={`bg-[#2a2a2a] rounded-xl overflow-hidden border border-gray-800 flex flex-col justify-between transition-all hover:border-purple-600 ${item.isAvailable === false ? 'opacity-50' : ''}`}>
                    <div>
                    ...
                    <div className="p-4 border-t border-gray-800 bg-[#1e1e1e]/50 flex justify-between items-center gap-2">
                      ...
                      <div className='flex gap-2'>
                        <button className="bg-purple-600 text-white p-2 rounded-lg hover:bg-purple-700" onClick={() => handleOpenModal(item)}><FaEdit /></button>
